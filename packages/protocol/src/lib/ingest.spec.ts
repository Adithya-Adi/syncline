...
import { isSynclineEvent, REQUEST_START, RRWEB_CUSTOM_EVENT_TYPE } from './events.js';
...
describe('syncline event detection', () => {
  ...
  it('ignores ordinary rrweb events and junk', () => {
    ...
    expect(
      isSynclineEvent({ type: RRWEB_CUSTOM_EVENT_TYPE, data: { tag: 'someone.else' } })
    ).toBe(false);
